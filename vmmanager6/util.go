...
//    * '_default' - sets the default log lefunc ConfigureLogger(enableOutput bool, logPath string, inputLogLevels map[string]string) {vel (if this is not set, the default is INFO)
...
func ConfigureLogger(enableOutput bool, logPath string, inputLogLevels map[string]string) {
        ...
        // for now though using just the consol     // translate the received log level into the zerolog Level type
	...
	mwriter := io.MultiWriter(f, origStdout)
        ...
        // this is mostly useful in this particular case to see logs from the vmmanager api library.
...
func resourceDataToFlatValues(d *schema.ResourceData, resource *schema.Resource) (map[string]interface{}, error) {
        ...
        for key, value := range resource.Schema {
                ...
                case schema.TypeSet:
                        flatValues[key] = d.Get(key).(interface{})
                case schema.TypeList:
                        flatValues[key] = d.Get(key)
                default:
